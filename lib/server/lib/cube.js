...
d3.json('http://sensor-bridge:3000/data?type=val&count=1', (d) => {
	for ( let i of d ){
  	drawGraph(handled(d))
  	make()
	}
...
function addData(name) {
  return context.metric(function (start, stop, step, callback) {
    d3.json('http://sensor-bridge:3000/data?type=val&count=' + datasize, (d) => {
      var values = []
      d = handled(d)
...
function make() {
  ...
  .data(["water_current", "water_level", "water_purity", "water_temp"].map(addData))
    .enter()
    .append("div")
    .attr("class", "horizon")
    .call(horizon);
