...
if [ ! -f 'raspbian-lite-latest.zip' ] ; then
	curl -L 'https://downloads.raspberrypi.org/raspbian_lite_latest' -o 'raspbian-lite-latest.zip'
fi
...
sudo sed -i "\$iif [ -e /setup1.sh ]; then sudo bash /setup1.sh $a ; fi" /mnt/sd/root/etc/rc.local
echo "$name" | sudo tee /mnt/sd/root/etc/hostname > /dev/null
sudo sed -i -e "s/raspberrypi/$name/" /mnt/sd/root/etc/hosts
sudo cp setup1.sh /mnt/sd/root/
